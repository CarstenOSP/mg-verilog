module forward_v9086_8_RAM_T2P_BRAM_1R1W #(parameterDataWidth    = 6,AddressRange = 3,AddressWidth = 2,BufferCount  = 2,MemLatency   = 1,IndexWidth   = 1) (input  wire                    i_ce,input  wire                    i_write,output wire                    i_full_n,input  wire                    i_ce0,input  wire                     i_we0,input  wire [AddressWidth-1:0] i_address0,input  wire [DataWidth-1:0]    i_d0,output wire [DataWidth-1:0]    i_q0,input  wire                    i_ce1,input  wire                     i_we1,input  wire [AddressWidth-1:0] i_address1,input  wire [DataWidth-1:0]    i_d1,input  wire                    t_ce,input  wire                    t_read,output wire                    t_empty_n,input  wire                    t_ce0,input  wire                    t_we0,input  wire [AddressWidth-1:0] t_address0,input  wire [DataWidth-1:0]    t_d0,output wire [DataWidth-1:0]    t_q0,input  wire                    t_ce1,input  wire                     t_we1,input  wire [AddressWidth-1:0] t_address1,input  wire [DataWidth-1:0]    t_d1,input  wire                    clk,input  wire                    reset);
...
forward_v9086_8_RAM_T2P_BRAM_1R1W_memcore forward_v9086_8_RAM_T2P_BRAM_1R1W_memcore_U (.ce0      ( buf_ce0[i] ),.we0      ( buf_we0[i] ),.address0 ( buf_a0[i] ),.d0       ( buf_d0[i] ),.q0       ( buf_q0[i] ),.ce1      ( buf_ce1[i] ),.we1      ( buf_we1[i] ),.address1 ( buf_a1[i] ),.d1       ( buf_d1[i] ),.clk      ( clk ),.reset    ( reset ));
    end
endgenerate
generate
