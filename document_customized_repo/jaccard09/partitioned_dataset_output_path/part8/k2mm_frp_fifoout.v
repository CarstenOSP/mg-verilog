...
module k2mm_frp_fifoout (ap_clk,ap_rst,data_in,data_in_vld,data_in_last,data_out,data_out_vld,ap_start,data_out_read,valid,num_valid_datasets,pf_ready,pf_done,pf_all_done,pf_continue); 
   parameter BlockingType = 1;  
   ...
   localparam NUM_STAGES = (PipeLatency+PipelineII-1) / PipelineII; 
   function isDefined;
      input val;
      begin
   ...
   reg [DataWidth-1+2:0] fifo [0:FDEPTH-1]; 
   reg write_enable, read_enable;
   reg fifo_empty, fifo_full, sel_pipe;
   reg signed [CeilLog2FDepth:0] fifo_rdPtr; 
   reg [CeilLog2FDepth:0] fifo_avail;
   reg signed [CeilLog2FDepth+1:0] ready_chk; 
   reg                  fail;
...
else if ( (data_out_vld&&data_out_read||!data_out_vld_fifo&&data_out_last)||(!pf_all_done||pf_all_done&&pf_continue) ) data_done <= 1'b1 & (BlockingType==1);
                else data_done <= 1'b0;
            end
    endgenerate
    generate
        if (PfAllDoneEnable == 2)
            ...
            always @ (*) begin : control
               ...
               casex ({data_out_vld_fifo, data_out_last, pf_continue&pf_all_done, stalled, data_out_read})
                 ...
                 5'b X_X_0_1_X : {internal_done, must_stall} = 2'b 1_1;  
                 5'b X_X_1_1_X : {internal_done, must_stall} = 2'b 1_0;  
                 5'b 0_1_0_0_X : {internal_done, must_stall} = 2'b 1_1;  
                 5'b 0_1_1_0_X : {internal_done, must_stall} = 2'b 1_0;
                 5'b 1_1_X_X_0 : {internal_done, must_stall} = 2'b 0_0;  
